...
class ImageNet_OSR(object):
    def __init__(self, known, unknown,dataroot='/data2/imagenet', use_gpu=True, num_workers=8, batch_size=128,
                 img_size=224):
        self.num_classes = len(known)
        self.known = known
        self.unknown = unknown# list(set(list(range(0, 1000))) - set(known))
        ...
        trainset = ImageNet_Filter(os.path.join(dataroot, 'train'), train_transform)
        print('All Train Data:', len(trainset))
        trainset.__Filter__(known=self.known)
        ...
        testset = ImageNet_Filter(os.path.join(dataroot, 'val'), transform)
        print('All Test Data:', len(testset))
        testset.__Filter__(known=self.known)
        ...
        outset = ImageNet_Filter(os.path.join(dataroot, 'val'), transform)
        outset.__Filter__(known=self.unknown)
...
class ImageNet_CLS(object):
    def __init__(self,known,dataroot='/data2/imagenet', use_gpu=True, num_workers=8, batch_size=128,
                 img_size=224):
        self.num_classes = len(known)
        self.known = known
        print('Selected Labels: ', known)

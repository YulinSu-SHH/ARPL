...
def compute_f1(pred_k, pred_u, labels,options):
    ...
    prod_k=F.softmax(torch.tensor(pred_k),dim=1)
    prod_u=F.softmax(torch.tensor(pred_u),dim=1)
   
    
    x1, prediction_k = torch.max(prod_k, 1)
    x2, prediction_u = torch.max(prod_u, 1)
    threshold=options['threshold']
    prediction_k[x1<threshold]=labelnum
    prediction_u[x2 < threshold] = labelnum
    tp=prediction_u.eq(labelnum).sum().item()
    fp=len(x2)-tp
    tn=prediction_k.eq(torch.tensor(labels)).sum().item()
    fn=len(x1)-tn
